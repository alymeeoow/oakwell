...
const Specialties = () => {
  ...
  const leftSpecialties = specialties.slice(0, 4);
  const rightSpecialties = specialties.slice(4, 6);
  ...
  <div className="specialties-left">
    <div className="box-grid-left">
      {leftSpecialties.map((specialty, index) => (
        <div 
          key={index} 
  ...
  SPECIAL<span className="t-logo-wrapper">
    ...
    <div className="box-grid-right">
      {rightSpecialties.map((specialty, index) => (
        <div 
          key={index + 4} 
          className={`box-item ${isVisible ? "animate-in" : ""}`}
